...
DrawerRviz::DrawerRviz(rclcpp::Node::SharedPtr &nh)
    : isfinished_(false)
    , isframerdy_(false)
    , ismaprdy_(false) {
    ...
    pose_pub_           = nh->create_publisher<nav_msgs::msg::Odometry>("pose", 2);
    path_pub_           = nh->create_publisher<nav_msgs::msg::Path>("path", 2);
    track_image_pub_    = nh->create_publisher<sensor_msgs::msg::Image>("tracking", 2);
    fixed_points_pub_   = nh->create_publisher<sensor_msgs::msg::PointCloud>("fixed", 2);
    current_points_pub_ = nh->create_publisher<sensor_msgs::msg::PointCloud>("current", 2);
}
...
void DrawerRviz::publishTrackingImage() {
    ...
    image.header.stamp    = rclcpp::Time::now();
