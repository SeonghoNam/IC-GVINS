...
GVINS2::GVINS2(const string &configfile, const string &outputpath, Drawer::Ptr drawer) {
    ...
    // Process threads
    fusion_thread_       = std::thread(&GVINS2::runFusion, this);
    tracking_thread_     = std::thread(&GVINS2::runTracking, this);
    optimization_thread_ = std::thread(&GVINS2::runOptimization, this);

    gvinsstate_ = GVINS_INITIALIZING;
}

bool GVINS2::addNewImu(const IMU &imu) {
...
bool GVINS2::addNewPVA(const PVA &pva)
{
    if (pva_buffer_mutex_.try_lock()) {
        pva_buffer_.push(pva);
        ispvaready_ = true;
...
bool GVINS2::addNewGnss(const GNSS &gnss) {
    // 低频观测, 无需加锁
...
bool GVINS2::addNewFrame(const Frame::Ptr &frame) {
...
void GVINS2::runFusion() {
    ...
    while (!isfinished_) { // While
        ...
        while (!pva_buffer_.empty()) { // IMU BUFFER
            // 读取IMU缓存
            // Load an IMU sample
            {
                Lock lock2(pva_buffer_mutex_);
                pva_pre = pva_cur;
                pva_cur = pva_buffer_.front();
                pva_buffer_.pop();
            }
            ...
            // INS mechanization
            { // INS
                Lock lock3(pva_mutex_);
                if (integration_config_.origin.isZero()) {
                    // 站心原点
                    // The origin of the world frame
                    integration_config_.origin       = pva_cur.blh;
                    integration_parameters_->gravity = Earth::gravity(pva_cur.blh);
                } 

                state.time = pva_cur.time;
                state.p    = Earth::global2local(integration_config_.origin, pva_cur.blh);
                state.q    = Rotation::euler2quaternion(pva_cur.att);
                state.v    = pva_cur.vel;

                pva_window_.emplace_back(pva_cur, state);


                if(gvinsstate_ == GVINS_INITIALIZING)
                {
                    if(ispvaready_ && state_mutex_.try_lock())
                    {
                        state = pva_window_.back().second;
                    ...
                    if ((isframeready_ || isgnssready_) && state_mutex_.try_lock()) {
                        if (isframeready_ && (keyframes_.front()->stamp() < pva_window_.back().second.time)) {
...
void GVINS2::runOptimization() {
    ...
    while (!isfinished_) {
        ...
        if (isgnssobs_ || isvisualobs_) {
            ...
            LOGE << "Optimization costs " << timecost.costInMillisecond() << " ms with " << timecosts_[0] << " and "
                 << timecosts_[1] << " with marginalization costs " << timecosts_[2];
        }
    }
}

void GVINS2::runTracking() {
    Frame::Ptr frame;
    Pose pose;
    ...
    while (!isfinished_) {
        ...
        while (!frame_buffer_.empty()) {
            ...
            // 读取缓存
            {
                ...
                pva_mutex_.lock();
                if (pva_window_.empty() || (pva_window_.back().second.time <= (frame->stamp() + td))) {
                    pva_mutex_.unlock();
                    frame_buffer_mutex_.unlock();
            ...
            {
                Lock lock2(pva_mutex_);
                frame->setStamp(frame->stamp() + td);
                frame->setTimeDelay(td);

                for(std::deque<std::pair<PVA, IntegrationState>>::reverse_iterator it = pva_window_.rbegin(); it != pva_window_.rend(); ++it) {
                    if(fabs(it->first.time - frame->stamp()) < 0.01) {
                        state = it->second;
                        break;
                    }
                }
                //`state = pva_window_.back().second;
                pose = MISC::stateToCameraPose(state, pose_b_c);
                // if(fabs(frame->stamp() - state.time) > 0.01)
                LOGI << frame->stamp() << " " << state.time;
                frame->setPose(pose);
            }
            ...
            if (tracking_->isNewKeyFrame() || (trackstate == TRACK_FIRST_FRAME) || trackstate == TRACK_LOST) {
                ...
                LOGI << frame->keyFrameId() << "-th keyframe Tracking " << timecost.costInMillisecond() << " ms";
            }
        }
    }
}

void GVINS2::setFinished() {
    isfinished_ = true;
...
void GVINS2::addNewKeyFrameTimeNode() {
    ...
    while (!keyframes_.empty()) {
        ...
        if (frametime > pva_window_.back().first.time) {
            break;
        }
...
bool GVINS2::removeUnusedTimeNode() {
...
bool GVINS2::insertNewGnssTimeNode() {
...
void GVINS2::addNewGnssTimeNode() {
...
void GVINS2::addNewTimeNode(double time) {
    ...
    for(std::deque<std::pair<PVA, IntegrationState>>::reverse_iterator it = pva_window_.rbegin(); it != pva_window_.rend(); ++it) {
        if(fabs(it->first.time - time) < 0.01) {
            state = it->second;
            break;
        }
    }
...
void GVINS2::parametersStatistic() {
...
bool GVINS2::gvinsOutlierCulling() {
...
void GVINS2::gnssOutlierCullingByChi2(ceres::Problem &problem,
...
int GVINS2::removeReprojectionFactorsByChi2(ceres::Problem &problem, vector<ceres::ResidualBlockId> &residual_ids,
                                           double chi2) {
    double cost;
...
void GVINS2::updateParametersFromOptimizer() {
    if (map_->keyframes().empty()) {
        return;
    }
...
bool GVINS2::gvinsRemoveAllSecondNewFrame() {
    vector<ulong> keyframeids = map_->orderedKeyFrames();
...
bool GVINS2::gvinsMarginalization() {
...
void GVINS2::addReprojectionParameters(ceres::Problem &problem) {
...
vector<ceres::ResidualBlockId> GVINS2::addReprojectionFactors(ceres::Problem &problem, bool isusekernel) {
...
int GVINS2::getStateDataIndex(double time) {
...
void GVINS2::addStateParameters(ceres::Problem &problem) {
...
void GVINS2::addImuFactors(ceres::Problem &problem) {
    for (size_t k = 0; k < preintegrationlist_.size(); k++) {
        // 预积分因子
...
void GVINS2::constructPrior(bool is_zero_velocity) {
