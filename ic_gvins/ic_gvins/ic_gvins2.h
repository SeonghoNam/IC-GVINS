...
#ifndef GVINS_GVINS2_H
#define GVINS_GVINS2_H
...
public:
    ...
    typedef std::shared_ptr<GVINS2> Ptr;
    typedef std::unique_lock<std::mutex> Lock;

    GVINS2() = delete;
    explicit GVINS2(const string &configfile, const string &outputpath, Drawer::Ptr drawer);

    bool addNewImu(const IMU &imu);
    bool addNewGnss(const GNSS &gnss);
    bool addNewFrame(const Frame::Ptr &frame);
    bool addNewPVA(const PVA &pva);             // by shnam

    void setFinished();

    bool isFrameBufferEmpty() const {
    ...
    void setWorldOrigin(Vector3d blh) {
        integration_config_.origin = blh;
    }
...
private:
    ...
    std::atomic<bool> ispvaready_{false};       // by shnam
    ...
    std::mutex pva_buffer_mutex_;
    std::mutex pva_mutex_;
    ...
    std::queue<PVA> pva_buffer_;                                // by shnam
    std::deque<std::pair<PVA, IntegrationState>> pva_window_;    // by shnam
